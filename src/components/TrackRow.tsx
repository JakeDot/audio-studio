import React, { useState } from 'react';
import { Volume2, VolumeX, Headphones, Mic2, Waves, Repeat } from 'lucide-react';
import { InstrumentType, TrackData } from '../types';
import { cn } from '../lib/utils';

interface TrackRowProps {
  track: TrackData;
  currentStep: number;
  bpm: number;
  onUpdate: (id: string, updates: Partial<TrackData>) => void;
  onDelete: (id: string) => void;
  onGenerateVocal?: (id: string, lyrics: string) => Promise<void>;
}

const INSTRUMENTS: { value: InstrumentType; label: string }[] = [
  { value: 'kick', label: 'Kick Drum' },
  { value: 'snare', label: 'Snare' },
  { value: 'hihat', label: 'Hi-Hat' },
  { value: 'bass', label: 'FM Bass' },
  { value: 'synth', label: 'AM Synth' },
  { value: 'pluck', label: 'Pluck' },
  { value: 'vocal', label: 'Vocal (AI)' },
];

export function TrackRow({ track, currentStep, bpm, onUpdate, onDelete, onGenerateVocal }: TrackRowProps) {
  const [isGenerating, setIsGenerating] = useState(false);

  const stepDuration = (60 / bpm) / 4;
  const vocalSteps = track.instrument === 'vocal' && track.vocalDuration 
    ? Math.max(1, Math.ceil(track.vocalDuration / stepDuration)) 
    : 1;

  const toggleStep = (index: number) => {
    const newSequence = [...track.sequence];
    newSequence[index] = !newSequence[index];
    onUpdate(track.id, { sequence: newSequence });
  };

  const handleGenerate = async () => {
    if (!onGenerateVocal || !track.lyrics) return;
    setIsGenerating(true);
    await onGenerateVocal(track.id, track.lyrics);
    setIsGenerating(false);
  };

  return (
    <div className="flex flex-col gap-2 p-3 bg-zinc-900 rounded-xl border border-zinc-800">
      <div className="flex items-center gap-4">
        {/* Track Controls */}
        <div className="flex flex-col gap-2 w-56 shrink-0">
          <div className="flex items-center justify-between">
            <input
              type="text"
              value={track.name}
              onChange={(e) => onUpdate(track.id, { name: e.target.value })}
              className="bg-transparent text-sm font-medium text-zinc-100 focus:outline-none w-24"
            />
            <button
              onClick={() => onDelete(track.id)}
              className="text-xs text-zinc-500 hover:text-red-400 transition-colors"
            >
              Del
            </button>
          </div>

          <select
            value={track.instrument}
            onChange={(e) => onUpdate(track.id, { instrument: e.target.value as InstrumentType })}
            className="bg-zinc-800 text-xs text-zinc-300 rounded px-2 py-1 border border-zinc-700 outline-none focus:border-indigo-500"
          >
            {INSTRUMENTS.map((inst) => (
              <option key={inst.value} value={inst.value}>
                {inst.label}
              </option>
            ))}
          </select>

          <div className="flex items-center gap-2">
            <button
              onClick={() => onUpdate(track.id, { muted: !track.muted })}
              className={cn(
                "p-1.5 rounded transition-colors",
                track.muted ? "bg-red-500/20 text-red-400" : "bg-zinc-800 text-zinc-400 hover:text-zinc-200"
              )}
              title="Mute"
            >
              {track.muted ? <VolumeX size={14} /> : <Volume2 size={14} />}
            </button>
            <button
              onClick={() => onUpdate(track.id, { solo: !track.solo })}
              className={cn(
                "p-1.5 rounded transition-colors",
                track.solo ? "bg-amber-500/20 text-amber-400" : "bg-zinc-800 text-zinc-400 hover:text-zinc-200"
              )}
              title="Solo"
            >
              <Headphones size={14} />
            </button>
            <input
              type="range"
              min="-60"
              max="6"
              step="1"
              value={track.volume}
              onChange={(e) => onUpdate(track.id, { volume: parseFloat(e.target.value) })}
              className="w-16 accent-indigo-500"
              title="Volume"
            />
          </div>
          
          {/* Effects Controls */}
          <div className="flex items-center gap-3 mt-1">
            <div className="flex items-center gap-1" title="Reverb">
              <Waves size={12} className="text-zinc-500" />
              <input
                type="range"
                min="0"
                max="1"
                step="0.01"
                value={track.reverb}
                onChange={(e) => onUpdate(track.id, { reverb: parseFloat(e.target.value) })}
                className="w-12 accent-cyan-500"
              />
            </div>
            <div className="flex items-center gap-1" title="Delay">
              <Repeat size={12} className="text-zinc-500" />
              <input
                type="range"
                min="0"
                max="1"
                step="0.01"
                value={track.delay}
                onChange={(e) => onUpdate(track.id, { delay: parseFloat(e.target.value) })}
                className="w-12 accent-purple-500"
              />
            </div>
          </div>
        </div>

        {/* Sequencer Grid */}
        <div className="flex-1 flex items-center gap-1 overflow-x-auto pb-2 sm:pb-0 h-full">
          {track.sequence.map((isActive, i) => {
            let isSpanned = false;
            let isStart = false;
            
            if (track.instrument === 'vocal' && track.vocalDuration) {
              for (let j = 0; j < vocalSteps; j++) {
                if (i - j >= 0 && track.sequence[i - j]) {
                  isSpanned = true;
                  if (j === 0) isStart = true;
                  break;
                }
              }
            } else {
              isSpanned = isActive;
              isStart = isActive;
            }

            return (
              <button
                key={i}
                onClick={() => toggleStep(i)}
                className={cn(
                  "h-16 flex-1 min-w-[2rem] rounded-md transition-all duration-75 border",
                  isSpanned 
                    ? track.instrument === 'vocal' 
                      ? "bg-pink-500 border-pink-400 shadow-[0_0_10px_rgba(236,72,153,0.5)]"
                      : "bg-indigo-500 border-indigo-400 shadow-[0_0_10px_rgba(99,102,241,0.5)]" 
                    : "bg-zinc-800 border-zinc-700 hover:bg-zinc-700",
                  currentStep === i && "ring-2 ring-white ring-offset-2 ring-offset-zinc-900",
                  i % 4 === 0 && !isSpanned && "bg-zinc-700/50",
                  isSpanned && !isStart && "opacity-50 shadow-none"
                )}
              />
            );
          })}
        </div>
      </div>

      {/* Vocal Input Section */}
      {track.instrument === 'vocal' && (
        <div className="flex items-center gap-2 mt-2 pt-2 border-t border-zinc-800">
          <Mic2 size={16} className="text-pink-400" />
          <input
            type="text"
            placeholder="Enter lyrics to sing..."
            value={track.lyrics || ''}
            onChange={(e) => onUpdate(track.id, { lyrics: e.target.value })}
            className="flex-1 bg-zinc-800 text-sm text-zinc-200 rounded px-3 py-1.5 border border-zinc-700 outline-none focus:border-pink-500"
          />
          <button
            onClick={handleGenerate}
            disabled={isGenerating || !track.lyrics}
            className="px-4 py-1.5 bg-pink-600 hover:bg-pink-500 disabled:bg-zinc-700 disabled:text-zinc-500 text-white text-sm font-medium rounded transition-colors"
          >
            {isGenerating ? 'Generating...' : 'Generate Voice'}
          </button>
        </div>
      )}
    </div>
  );
}
